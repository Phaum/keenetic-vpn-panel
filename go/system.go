...
stop() { [ -f "$PID_FILE" ] || return 0; PID="$(cat "$PID_FILE")"; kill "$PID" 2>/dev/null || true; COUNT=0; while kill -0 "$PID" 2>/dev/null && [ "$COUNT" -lt 10 ]; do sleep 1; COUNT=$((COUNT+1)); done; rm -f "$PID_FILE"; }
