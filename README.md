...
- Фоновый автоматический режим проверки с настраиваемым интервалом
...
- `generated/adguardvpn-rotate.sh` — Авто исполнитель для запуска Python-ротации
...
Сгенерированный файл `generated/adguardvpn-rotate.sh` остаётся как совместимый авто исполнитель для cron, init.d и ручного запуска на роутере.
...
В `config.json` теперь есть отдельная секция:
...
- `enabled` включает или отключает фоновую автопроверку
- `check_interval` задаёт интервал между циклами в секундах

Когда автоматический режим включён, запущенная панель сама выполняет периодическую проверку.
Если ресурс недоступен, автоматически запускается Python-native ротация локаций.

Переключать этот режим можно прямо из web-интерфейса на странице обзора и в настройках.
...
После успешного обновления веб-панель автоматически перезапускается, чтобы сразу подняться уже на новой версии кода.
...
Важно: автозапуск Entware и автоматический режим проверки это разные вещи.

- `autostart` отвечает за запуск самой веб-панели как сервиса после перезагрузки
- `automation` отвечает за фоновую периодическую проверку внутри уже запущенной панели
